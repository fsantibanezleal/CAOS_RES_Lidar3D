...
| M7 | **pretrained backbone** (v0.11.x) | **resnet18** (12.8 M) | TUM RGB-D **+ ICL-NUIM** (perfect GT depth), 7329 pairs | *in training* (epoch 0 = 0.60 m; best-checkpoint over 10 epochs) | pending verify | ImageNet ResNet-18 shared by depth decoder + Siamese pose head; pose loss ~0.0015 (much steadier than scratch). Decision: Felipe chose the pretrained-backbone path for real depth-quality gains. Deploy only if it verifies sharper than M4 |
