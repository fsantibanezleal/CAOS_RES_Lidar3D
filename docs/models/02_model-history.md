...
|---|---|---|---|---|---|---|
| M1 | first own model (v0.06.000) | scratch (2.2 M) | TUM RGB-D, 5 seq (~9.9k frames) | **~0.20 m** | yes (superseded) | first honest from-scratch depth+pose; baked our desk case |
...
| M4 | early-stopping retrain (v0.09.004) | scratch | TUM RGB-D | **0.29 m** | **yes (LIVE)** | added best-checkpoint early stopping; re-baked our case (coherent per-frame depth, more structure). **This baked artifact is what the site currently serves.** |
...
Our case reconstructs a short (~60-frame) desk sweep, so its accumulated drift is far below the ~300-frame
...
Deploy is gated on *verified improvement*: a new model is baked into our case and screenshot-compared against the
