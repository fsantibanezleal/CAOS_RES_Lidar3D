...
def _synthetic_scans(spec: SequenceSpec, seed: int) -> tuple[list[np.ndarray], np.ndarray]:
    ...
    S = int(max(6, min(spec.max_frames, 36)))
...
def reconstruct(spec: SequenceSpec, seed: int = 42) -> ReconResult:
    ...
    poses12 = np.asarray([np.concatenate([P[:3, :3].reshape(-1), P[:3, 3]]).reshape(3, 4).reshape(-1)
                          for P in poses], np.float32)
    return ReconResult(
        ...
        bbox_min=pts.min(0).tolist(), bbox_max=pts.max(0).tolist(), depth_thumbs=[],
    )
