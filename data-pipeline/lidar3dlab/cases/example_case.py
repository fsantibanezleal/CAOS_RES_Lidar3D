...
CASES: list[Case] = [
    ...
    Case("DICP_tum_office", "classical: depth-only ICP (no RGB in the pose)",
         ...
         "real", dataset="TUM RGB-D (freiburg3_long_office_household, Sturm et al. 2012)",
         license="CC BY 4.0 (TUM RGB-D)"),
    Case("DICP_tum_desk2", "classical: depth-only ICP (no RGB in the pose)",
         SequenceSpec("DICP_tum_desk2",
                      source_dir=str(DATA_ROOT / "train" / "tum-rgbd" / "rgbd_dataset_freiburg2_desk"),
                      n_frames=0, max_frames=240, decimation=2, engine="depth-icp",
                      intrinsics=_TUM2, max_render_depth=6.0),
         "the SAME wide desk loop, pose from the Kinect depth alone (point-to-plane ICP): the classical baseline",
         "real", dataset="TUM RGB-D (freiburg2_desk, Sturm et al. 2012)", license="CC BY 4.0 (TUM RGB-D)"),
    Case("DICP_tum_xyz", "classical: depth-only ICP (no RGB in the pose)",
         SequenceSpec("DICP_tum_xyz",
                      source_dir=str(DATA_ROOT / "train" / "tum-rgbd" / "rgbd_dataset_freiburg1_xyz"),
                      n_frames=0, max_frames=240, decimation=2, engine="depth-icp",
                      intrinsics=_TUM1, max_render_depth=5.0),
         "the SAME xyz sweep, pose from the Kinect depth alone (point-to-plane ICP): the classical baseline",
         "real", dataset="TUM RGB-D (freiburg1_xyz, Sturm et al. 2012)", license="CC BY 4.0 (TUM RGB-D)"),
    Case("DICP_tum_pioneer", "classical: depth-only ICP (no RGB in the pose)",
         SequenceSpec("DICP_tum_pioneer",
                      source_dir=str(DATA_ROOT / "train" / "tum-rgbd" / "rgbd_dataset_freiburg2_pioneer_slam"),
                      n_frames=0, max_frames=240, decimation=2, engine="depth-icp",
                      intrinsics=_TUM2, max_render_depth=6.0),
         "the SAME robot SLAM run, pose from the Kinect depth alone (point-to-plane ICP): the classical baseline",
         "real", dataset="TUM RGB-D (freiburg2_pioneer_slam, Sturm et al. 2012)", license="CC BY 4.0 (TUM RGB-D)"),
]
