...
export const MODEL_HISTORY: ModelRun[] = [
  ...
  },
  {
    ...
    notes_en: 'The two-track family goes live: SIFT + PnP on the real Kinect depth (metric by construction, the scale blocker disappears at the source) + the M-C windowed pose-graph fusing the strong metric edges (a further 7-26% drift cut; its first production use). Office 0.085 m, desk 0.034, pioneer 0.024 against 0.28 m RGB-only, measured standalone at 300/120/150 frames before the depth-edge guard shipped; the pipeline measures the shipped configuration itself since 0.16.000 (0.014-0.040 m over five scenes at 240 frames). Cases RGBD_tum_office/desk mirror the RGB-only OWN_* scenes for an honest side-by-side. Sensor holes stay holes; nothing is hallucinated.',
    notes_es: 'La familia de dos tracks entra en producción: SIFT + PnP sobre la profundidad Kinect real (métrica por construcción, el bloqueador de escala desaparece en el origen) + el pose-graph por ventanas M-C fusionando las aristas métricas fuertes (un recorte adicional de drift de 7-26%; su primer uso productivo). Office 0.085 m, desk 0.034, pioneer 0.024 frente a 0.28 m RGB-only, medidos de forma aislada a 300/120/150 fotogramas y antes de la guarda de bordes de profundidad; desde 0.16.000 el pipeline mide por si mismo la configuracion desplegada (0.014-0.040 m en cinco escenas a 240 fotogramas). Los casos RGBD_tum_office/desk reflejan las escenas RGB-only OWN_* para una comparación honesta lado a lado. Los huecos del sensor quedan como huecos; nada se alucina.',
  },
  {
