...
export function Methodology({ lang }: { lang: Lang }) {
  ...
  const sota = (
    <>
      ...
      <p>{en
        ...
        : 'El SLAM visual clásico (ORB-SLAM3, DROID-SLAM) y la odometría LiDAR (LOAM, KISS-ICP) recuperan geometría optimizando: extraer features, emparejarlos y minimizar un residual de reproyección o punto-a-plano sobre un grafo de poses. Es preciso pero iterativo, sensible a la inicialización y difícil de ejecutar causalmente a frame rate en secuencias largas.'}<Cite k="orbslam3" /><Cite k="droid" /><Cite k="kissicp" /></p>
      <p>{en
  ...
  const context = (
    <>
      ...
      <ul>
        ...
        <li><b>{en ? 'Trajectory memory' : 'Memoria de trayectoria'}:</b> {en ? 'all older frames are compressed to 6 tokens each (ordered by Video-RoPE) for cheap long-range drift correction.' : 'todos los cuadros viejos se comprimen a 6 tokens cada uno (ordenados por Video-RoPE) para corrección barata del drift de largo rango.'}<Cite k="rope" /></li>
      </ul>
  ...
  const ownmodel = (
    <>
      <h3>{en ? 'Estela: our own trainable depth+pose model' : 'Estela: nuestro modelo entrenable de profundidad+pose'}</h3>
      <p>{en
        ? 'Alongside the vendored SOTA engine, this lab ships its own model, trainable from scratch and model-agnostic behind a single reconstruct() contract. It predicts, per frame, a dense metric depth with a learned confidence, and, per consecutive pair, a 6-DoF relative pose. Two interchangeable backbones sit behind one forward signature: a from-scratch UNet (the honest baseline) and a pretrained ImageNet ResNet-18 (the quality variant, shared by the depth decoder and a Siamese pose head). The ResNet backbone is a generic vision feature extractor, not a third-party reconstruction product; the decoder, the confidence head, the pose head, the se(3) math and the training loop are ours.'
        : 'Junto al motor SOTA vendorizado, este lab trae su propio modelo, entrenable desde cero y agnóstico al modelo tras un único contrato reconstruct(). Predice, por cuadro, una profundidad métrica densa con confianza aprendida, y, por par consecutivo, una pose relativa de 6 grados de libertad. Dos backbones intercambiables tras una misma firma: un UNet desde cero (la línea base honesta) y un ResNet-18 preentrenado en ImageNet (la variante de calidad, compartido por el decoder de profundidad y una cabeza de pose Siamese). El backbone ResNet es un extractor de features genérico, no un producto de reconstrucción de terceros; el decoder, la cabeza de confianza, la de pose, el álgebra se(3) y el entrenamiento son nuestros.'}<Cite k="resnet" /></p>
      <p>{en ? 'The depth head is aleatoric: it emits a metric depth and a log-variance, trained with a heteroscedastic negative log-likelihood so the model can down-weight pixels it cannot predict (thin structure, reflections). The confidence ' : 'La cabeza de profundidad es aleatoria: emite una profundidad métrica y una log-varianza, entrenada con una verosimilitud negativa heterocedástica para que el modelo baje el peso de los píxeles que no puede predecir (estructura fina, reflejos). La confianza '}<Katex tex={String.raw`c=e^{-\log\hat\sigma^2}`} />{en ? ' both weights the loss and filters the cloud:' : ' pondera la pérdida y filtra la nube:'}</p>
      <Katex block tex={String.raw`\mathcal{L}_{D}=\frac{1}{|\Omega|}\sum_{p\in\Omega}\Big(\tfrac{1}{2}e^{-\log\hat\sigma^2_p}\,\lVert \hat D_p - D_p\rVert^2 + \tfrac{1}{2}\log\hat\sigma^2_p\Big)`} />
      <p>{en ? 'The pose head regresses an se(3) tangent ' : 'La cabeza de pose regresa un tangente se(3) '}<Katex tex={String.raw`\xi=(\boldsymbol\omega,\mathbf v)`} />{en ? ' from the pooled features of both frames; our own Rodrigues exponential maps it to a transform:' : ' desde las features agrupadas de ambos cuadros; nuestra exponencial de Rodrigues lo lleva a una transformada:'}</p>
      <Katex block tex={String.raw`\mathbf R=\mathbf I+\sin\theta\,\mathbf K+(1-\cos\theta)\,\mathbf K^2,\quad \theta=\lVert\boldsymbol\omega\rVert,\ \mathbf K=[\hat{\boldsymbol\omega}]_\times`} />
      <p className="muted">{en ? 'Trained on real TUM RGB-D (11 sequences) + ICL-NUIM synthetic perfect-depth; best held-out ATE about 0.37 m. Every training epoch is logged (Experiments -> Model history), so no experiment is lost.' : 'Entrenado en TUM RGB-D real (11 secuencias) + ICL-NUIM sintético de profundidad perfecta; mejor ATE held-out cerca de 0.37 m. Cada época queda registrada (Experimentos -> Historial de modelos), sin perder experimentos.'}<Cite k="tum" /><Cite k="iclnuim" /></p>

      <h3>{en ? 'The pose-refinement ladder (clean-map stack)' : 'El ladder de refinamiento de pose (stack de mapa limpio)'}</h3>
      <p>{en ? 'The per-frame depth is sharp, but raw pose accumulation drifts. So the engine has a three-rung refinement ladder, each falling back to the previous:' : 'La profundidad por-cuadro es nítida, pero la acumulación cruda de pose deriva. Por eso el motor tiene un ladder de tres niveles, cada uno con fallback al anterior:'}</p>
      <ul>
        <li><b>{en ? 'Frame-to-frame ICP' : 'ICP cuadro-a-cuadro'} <Cite k="open3d" />:</b> {en ? 'each predicted relative pose is refined by point-to-plane ICP on the depth clouds (model pose as init, guarded). Removes most local drift. This is the shipped default.' : 'cada pose relativa predicha se refina con ICP punto-a-plano sobre las nubes de profundidad (init con la pose del modelo, protegido). Quita la mayor parte del drift local. Es el default desplegado.'}<Katex tex={String.raw`\min_{\mathbf T}\sum_i\big((\mathbf T\mathbf p_i-\mathbf q_i)\cdot\mathbf n_i\big)^2`} /></li>
        <li><b>{en ? 'D1: global pose-graph + loop closure' : 'D1: pose-graph global + loop closure'} <Cite k="open3d" />:</b> {en ? 'Open3D multiway registration builds odometry + loop-closure edges and distributes drift globally so revisited places snap together (a bundle-adjustment-style fix).' : 'la registración multiway de Open3D arma aristas de odometría + loop-closure y distribuye el drift globalmente para que los lugares revisitados encajen (un arreglo estilo bundle adjustment).'}</li>
        <li><b>{en ? 'TSDF volumetric fusion' : 'Fusión volumétrica TSDF'}:</b> {en ? 'integrate confident, near depth into a truncated signed-distance volume and extract one denoised surface (KinectFusion-style); volumetric averaging cancels per-frame depth noise.' : 'integrar profundidad confiable y cercana en un volumen de distancia con signo truncada y extraer una superficie sin ruido (estilo KinectFusion); el promediado volumétrico cancela el ruido de profundidad por-cuadro.'}</li>
      </ul>
      <p className="muted">{en
        ? 'D1 and TSDF are the right tools for a pristine surface but need sub-voxel pose accuracy. At the current monocular accuracy (~0.37 m ATE) they over-constrain / fuse sparsely, so ICP-refined accumulation stays the default; both are one flag away and become the default once a stronger pose model (more data) lands. The per-frame depth is excellent; the fused map is bounded by pose accuracy, which is the documented limit of feed-forward monocular reconstruction and the target of the novel agenda.'
        : 'D1 y TSDF son las herramientas correctas para una superficie prístina pero necesitan precisión de pose sub-vóxel. A la precisión monocular actual (~0.37 m ATE) sobre-restringen / fusionan de forma dispersa, así que la acumulación refinada por ICP sigue de default; ambos están a un flag y serán el default cuando llegue un modelo de pose más fuerte (más datos). La profundidad por-cuadro es excelente; el mapa fusionado está acotado por la precisión de pose, el límite documentado de la reconstrucción monocular feed-forward y el objetivo de la agenda novel.'}<Cite k="umeyama" /></p>
    </>
  );
  ...
  const geometry = (
    <>
      ...
      <p>{en ? 'Concatenating the kept points over all frames gives the RGB world cloud replayed in the App; the per-frame counts are stored as ' : 'Concatenar los puntos conservados sobre todos los cuadros da la nube RGB de mundo que se reproduce en la App; los conteos por cuadro se guardan como '}<Katex tex={String.raw`\text{frame\_offsets}`} />{en ? ' so the web can reveal the reconstruction in exact frame order.' : ' para que la web revele la reconstrucción en orden exacto de cuadro.'}</p>
  ...
  return (
    <div className="page">
      ...
      <p className="lead">{en
        ? 'Two camera engines: the vendored SOTA reference (the Geometric Context Transformer, a frozen-DINOv2 ViT with 24 alternating attention blocks) and Estela, our own trainable depth+pose model (a pretrained ResNet-18 backbone with our decoder, Siamese pose head and an ICP / global-pose-graph / TSDF refinement ladder). This page covers the state of the art, the networks, our model, the attention context, and the geometry.'
        : 'Dos motores de cámara: la referencia SOTA vendorizada (el Geometric Context Transformer, un ViT con DINOv2 congelado y 24 bloques de atención alternada) y Estela, nuestro modelo entrenable de profundidad+pose (un backbone ResNet-18 preentrenado con nuestro decoder, cabeza de pose Siamese y un ladder de refinamiento ICP / pose-graph global / TSDF). Esta página cubre el estado del arte, las redes, nuestro modelo, el contexto de atención y la geometría.'}</p>
      <SubTabs tabs={[
        ...
        { id: 'own', label: en ? 'Our model' : 'Nuestro modelo', body: <>{ownmodel}<Refs ids={['resnet', 'tum', 'iclnuim', 'open3d', 'umeyama']} /></> },
