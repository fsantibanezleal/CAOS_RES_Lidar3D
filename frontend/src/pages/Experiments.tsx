...
export function Experiments({ lang }: { lang: Lang }) {
  ...
  const models = (
    <>
      ...
      <p>{en
        ? 'The complete, honest record of every model trained for our engine: backbone, data, held-out ATE (RMS trajectory error in metres on TUM long-office, Umeyama-aligned; lower is better), whether it was deployed, and every negative result. Kept in sync with the repo docs and the append-only experiments log.'
        : 'El registro completo y honesto de cada modelo entrenado para el motor propio: backbone, datos, ATE held-out (error RMS de trayectoria en metros en TUM long-office, alineado con Umeyama; menor es mejor), si se desplegó, y cada resultado negativo. Sincronizado con los docs del repo y el log append-only de experimentos.'}</p>
      ...
      <p className="muted">{en
        ...
        : 'El aspecto difuso-vs-nítido lo domina el drift de pose, no el ruido de profundidad: la profundidad por cuadro ya es coherente. El early-stopping por mejor checkpoint ayudó (M2→M4), las pérdidas extra dañaron (M5), y el backbone preentrenado (M7) es la palanca elegida: un encoder más fuerte más una cabeza de pose Siamese más estable atacan el drift directamente. El despliegue se decide por comparación visual verificada, nunca por el número de ATE solo.'}</p>
    </>
  );
  ...
  const cases = (
    <>
      ...
      <ul>
        ...
        <li><b>oxford / university / loop / courthouse</b> <span className="tag">{en ? 'real, GPU' : 'real, GPU'}</span>: {en ? 'real camera sequences baked offline on an 8 GB GPU: an outdoor walk, a courtyard, a revisit (the loop-closure showcase), and a facade orbit.' : 'secuencias reales de cámara precalculadas offline en una GPU de 8 GB: una caminata exterior, un patio, un revisit (la vitrina de loop-closure) y una órbita de fachada.'}</li>
  ...
  const config = (
    <>
      ...
      <p>{en
        ...
        : 'En una RTX 4070 Laptop (8 GB), lingbot-map se ejecuta con el backend SDPA (sin compilar FlashInfer), CPU-offload, una ventana KV deslizante reducida (16), una sola iteración de cámara, bf16 y un tope de cuadros. Pico verificado cerca de 7.1 GB. El precálculo de oxford produjo una nube RGB de 193k puntos sobre 48 cuadros con una trayectoria de 3.13 m.'}<Cite k="flashinfer" /></p>
  ...
  return (
    <div className="page">
      ...
      <p className="lead">{en
        ...
        : 'Los casos cubren categorías: un control sintético CPU (el smoke de CI), el motor LiDAR y secuencias reales de cámara precalculadas offline en una GPU de 8 GB. Esta página también expone la agenda novel evaluada más allá del motor SOTA.'}</p>
      <SubTabs tabs={[
