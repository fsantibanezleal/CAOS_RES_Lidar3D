  ...
  const cases = (
    <>
      ...
      <ul>
        ...
        <li><b>RGBD_tum_office / RGBD_tum_desk</b> <span className="tag">{en ? 'Track B: RGB + sensor depth' : 'Track B: RGB + profundidad de sensor'}</span>: {en ? 'the SAME scenes as OWN_tum_office/desk, but integrating the real Kinect depth: SIFT + PnP geometric pose on metric sensor depth + the windowed pose-graph fusion (0.024-0.085 m vs 0.28 m RGB-only). The honest side-by-side of what a depth sensor buys.' : 'las MISMAS escenas que OWN_tum_office/desk, pero integrando la profundidad Kinect real: pose geométrica SIFT + PnP sobre profundidad métrica del sensor + la fusión pose-graph por ventanas (0.024-0.085 m vs 0.28 m solo-RGB). La comparación honesta de lo que aporta un sensor de profundidad.'}</li>
      </ul>
