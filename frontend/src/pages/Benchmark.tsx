...
export function Benchmark({ lang }: { lang: Lang }) {
  ...
  const ladder = (
    <>
      ...
      <table className="data"><thead>
        ...
        <tr><td>{en ? 'RGB-D sensor geometry (Track B)' : 'Geometría RGB-D de sensor (Track B)'}</td><td><span className="tag classical">{en ? 'classical' : 'clásico'}</span></td><td>{en ? 'SIFT + depth-edge guard + PnP on Kinect depth + windowed pose-graph fusion + TSDF surface fusion; metric by construction (no monocular scale ambiguity)' : 'SIFT + guard de bordes de profundidad + PnP sobre profundidad Kinect + fusión pose-graph por ventanas + fusión de superficie TSDF; métrico por construcción (sin ambigüedad de escala monocular)'}</td><td>{en ? 'wired + LIVE (RGBD cases; 0.014-0.040 m after the I1 guard, TSDF-denoised cloud, vs 0.28 m RGB-only)' : 'integrado + EN VIVO (casos RGBD; 0.014-0.040 m tras el guard I1, nube denoised por TSDF, vs 0.28 m RGB-only)'}</td></tr>
  ...
  const measured = (
    <>
      ...
      <p className="hint">{en ? 'From the committed manifests (RTX 4070 Laptop, 8 GB-safe config: SDPA, CPU offload, sliding KV window). We report what we can measure without ground truth.' : 'De los manifiestos confirmados (RTX 4070 Laptop, config segura 8 GB: SDPA, offload a CPU, ventana KV deslizante). Reportamos lo que podemos medir sin referencia.'}</p>
      <table className="data"><thead>
  ...
  const honesty = (
    <>
      ...
      <ul>
        ...
        <li><b>{en ? 'Measured' : 'Medido'}:</b> {en ? 'points, frames, path length, VRAM and artifact size come from our own committed manifests.' : 'puntos, cuadros, longitud de trayecto, VRAM y tamaño de artefacto vienen de nuestros propios manifiestos confirmados.'}</li>
