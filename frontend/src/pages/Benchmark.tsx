  ...
  const ladder = (
    <>
      ...
      <table className="data"><thead>
        ...
        <tr><td>{en ? 'RGB-D sensor geometry (Track B)' : 'Geometría RGB-D de sensor (Track B)'}</td><td><span className="tag classical">{en ? 'classical' : 'clásico'}</span></td><td>{en ? 'SIFT + PnP on Kinect depth + windowed pose-graph fusion; metric by construction (no monocular scale ambiguity)' : 'SIFT + PnP sobre profundidad Kinect + fusión pose-graph por ventanas; métrico por construcción (sin ambigüedad de escala monocular)'}</td><td>{en ? 'wired + LIVE (RGBD cases; 0.024-0.085 m vs 0.28 m RGB-only)' : 'integrado + EN VIVO (casos RGBD; 0.024-0.085 m vs 0.28 m RGB-only)'}</td></tr>
