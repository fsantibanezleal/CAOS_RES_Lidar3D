...
export function ArchModal({ lang, onClose }: { lang: Lang; onClose: () => void }) {
  ...
  const tabs: { label: string; diagram: JSX.Element; body: string }[] = [
    { label: en ? 'The app' : 'La app', diagram: <DesignFlowDiagram lang={lang} />,
      body: en ? 'Lidar 3D turns a video or LiDAR stream into camera poses, dense metric depth, and a colored world point cloud. The diagram shows both what it IS and how it was designed and built: research, implement, train and validate, bake a committed artifact, build the SPA, deploy. The App you are in replays a committed artifact; it does not recompute in the browser.'
                : 'Lidar 3D convierte un stream de video o LiDAR en poses de cámara, profundidad métrica densa y una nube de puntos de mundo coloreada. El diagrama muestra qué ES y cómo se diseñó y construyó: research, implementar, entrenar y validar, hornear un artefacto commiteado, construir la SPA, desplegar. La App reproduce un artefacto commiteado; no recomputa en el browser.' },
    { label: en ? 'Lanes' : 'Carriles', diagram: <LanesDiagram lang={lang} />,
      body: en ? 'Three lanes (ADR-0057). OFFLINE (precompute, GPU): the engine bakes a committed artifact. REPLAY (this static web): loads only committed artifacts and replays them. LIVE (local-GPU API, dormant): real-time reconstruction of your own footage on a machine with the GPU. The split of what runs in the web vs offline vs compute is explicit.'
                : 'Tres carriles (ADR-0057). OFFLINE (precómputo, GPU): el motor hornea un artefacto commiteado. REPLAY (esta web estática): carga solo artefactos commiteados y los reproduce. LIVE (API GPU local, latente): reconstrucción en vivo de tu propio video en una máquina con GPU. La separación web vs offline vs cómputo es explícita.' },
